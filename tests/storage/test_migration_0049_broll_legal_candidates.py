...
MIGRATION_PATH = Path(
    "packages/core/storage/alembic/versions/0049_broll_legal_candidates.py"
)
...
def _load_migration():
    spec = importlib.util.spec_from_file_location("_mig_0049", MIGRATION_PATH)
...
def test_migration_revision_chains_to_single_head() -> None:
    ...
    assert 'revision = "0049_broll_legal_candidates"' in source
    assert 'down_revision = "0048_emphasis_floor_prompts"' in source
    assert len("0049_broll_legal_candidates") <= 32
