...
- DB schema 迁移**只**在 `packages/core/storage/alembic/versions/`（当前 `0001…0061`，单一 head `0061_drop_v1_prompts`；`0058` 发布 BGM Agent prompt，`0059` 收敛 CreativeIntent 字幕提示，`0060` 清理旧字幕/花字/PostProcess 数据，`0061` 删除 v1 Agent prompt 与诊断；历史 prompt 迁移必须内联冻结、不得读取可变 seed JSON；`0014` 合并过早期双 `0012` 分支，两个 `0018` 文件是线性顺接、非分叉）。
