...
- DB schema 迁移**只**在 `packages/core/storage/alembic/versions/`（当前 `0001…0064`，单一 head `0064_caption_style_intensity`；`0058` 增加可恢复上传，`0059` 发布 BGM Agent prompt，`0060` 收敛 CreativeIntent 字幕提示，`0061` 清理旧字幕/花字/PostProcess 数据，`0062` 删除 v1 Agent prompt 与诊断，`0063` 持久化 run 取消模式与请求时间，`0064` 新增不可变 CreativeIntent v2 强度 prompt 并重绑；历史 prompt 迁移必须内联冻结、不得读取可变 seed JSON；`0014` 合并过早期双 `0012` 分支，两个 `0018` 文件是线性顺接、非分叉）。
