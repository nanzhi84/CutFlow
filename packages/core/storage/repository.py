...
_CREATIVE_INTENT_V1_CONTENT = (
    ...
    "- emphasis：对象数组，可为空 []。每项只能包含 phrase、priority、display_mode；"
    "phrase 必须逐字取自脚本原文连续子串；priority 为 0 到 100 整数；"
    "display_mode 只能是 inline 或 whole_cue。inline 表示句内局部强调，"
    "whole_cue 表示整条短 cue 在固定字幕带内强调；不要输出位置、坐标、字体或动画。\n\n"
    "脚本：\n"
    "{script}"
)

_CREATIVE_INTENT_V2_CONTENT = (
    "你是资深短视频创意策划。基于下面的口播脚本，提炼创意结构。\n\n"
    "严格要求：直接输出一个 JSON 对象（以左花括号开头、右花括号结尾）；"
    "禁止使用 markdown 代码块；禁止任何前后缀说明文字。\n\n"
    "JSON 必须且只能包含以下字段：\n"
    ...
    "俏皮 中选择一个，用来指导背景音乐精确匹配；不要输出枚举外词。\n"
    "- emphasis：对象数组，可为空 []。每项只能包含 phrase、priority、display_mode、"
    "intensity；phrase 必须逐字取自脚本原文连续子串；priority 为 0 到 100 整数；"
    "display_mode 只能是 inline 或 whole_cue。inline 表示句内局部强调，"
    "whole_cue 表示整条短 cue 在固定字幕带内强调；intensity 只能是 normal、strong "
    "或 hero，单条视频 hero 最多 1 个、strong 最多 3 个，其余使用 normal；"
    "不要输出位置、坐标、字体或动画。\n\n"
    "脚本：\n"
    "{script}"
)
...
class Repository:
    ...
    def seed(self) -> None:
        ...
        version_v1 = PromptVersion(
            id="prompt_creative_intent_v1",
            prompt_template_id=template.id,
            content=_CREATIVE_INTENT_V1_CONTENT,
            status="published",
            approved_at=utcnow(),
            published_at=utcnow(),
        )
        version_v2 = PromptVersion(
            id="prompt_creative_intent_v2",
            prompt_template_id=template.id,
            content=_CREATIVE_INTENT_V2_CONTENT,
            status="published",
            approved_at=utcnow(),
            published_at=utcnow(),
        )
        binding = PromptBinding(
            ...
            prompt_template_id=template.id,
            prompt_version_id=version_v2.id,
            node_id="ResolveCreativeIntent",
            priority=1,
        )
        self.prompt_templates[template.id] = template
        self.prompt_versions[version_v1.id] = version_v1
        self.prompt_versions[version_v2.id] = version_v2
