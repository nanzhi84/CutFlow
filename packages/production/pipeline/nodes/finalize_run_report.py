...
def run(ctx: NodeContext) -> NodeOutput:
    ...
    # §6.6 commit -> release: mark every shipped asset as ``committed`` for audit,
    # then release this run's remaining uncommitted shortlist leases so a sibling run
    # can claim them. Future "used" pressure comes from the selection ledger.
    try:
