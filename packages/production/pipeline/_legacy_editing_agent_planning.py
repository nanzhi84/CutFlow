"""Frozen v1 combined media/BGM editing-agent compatibility service.

The legacy LLM selects portrait, B-roll, and BGM IDs; local materializers turn
the media choices into frame-exact artifacts. Its wrapper then layers the frozen
v1 Huazi/style compatibility behavior on top. New workflows must not use it.
...
def _retrieval_topk_by_window(retrieval: dict | None) -> dict[str, list[str]]:
    candidates_by_window = (
        retrieval.get("candidates_by_window") if isinstance(retrieval, dict) else {}
    )
...
def _limited_ids(
    values: list[str] | None, *, limit: int = _PROMPT_RETRIEVAL_TOPK_LIMIT
) -> list[str]:
    ids: list[str] = []
...
def _compact_prompt_input(agent_input: dict, *, include_bgm: bool = True) -> dict:
    ...
    if not include_bgm:
        payload.pop("bgm_candidates", None)
    return payload
    ...
    *,
    ...
    def usable(candidate_id: str, slot: dict) -> bool:
        ...
        if candidate is None:
            return False
        if _agent_slot_has_retrieval_constraint(
            slot, retrieval_topk_by_window
    ...
    for choice in selection.portrait:
        ...
        choice_is_valid = (
            slot is not None and choice.slot_id not in used_slots and usable(choice.window_id, slot)
        )
        if choice_is_valid:
...
def _repair_broll_selection_to_constraints(
    *,
    ...
    def usable(candidate_id: str, slot: dict) -> bool:
        ...
        if not allow_asset_diversity_reuse and diversity_key and diversity_key in used_diversity:
            return False
...
def select_editing_assignment(
    *,
    ...
    prompt_node_id: str = "EditingAgentPlanning",
    idempotency_scope: str = "editing_agent",
    ...
    bgm_enabled = state.request.bgm.enabled if bgm_enabled is None else bool(bgm_enabled)
    ...
    if profile is None:
        ...
        prompt_input = _compact_prompt_input(agent_context.agent_input, include_bgm=bgm_enabled)
...
def materialize_media_selection_outputs(
    *,
    ...
    assignment_bgm_id: str | None = None,
) -> MediaSelectionMaterializedOutputs:
    """Materialize only portrait/B-roll decisions from the media-selection pass.

    This is the shared seam used by the legacy v1 wrapper and the v2
    ``MediaSelectionAgentPlanning`` node. It intentionally does not inspect creative
    intent, plan captions/BGM, or emit ``plan.style``.
    """
