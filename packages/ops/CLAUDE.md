...
## 职责
- 指标计算（纯函数，均在 `__init__.py` 导出）：`cost_metrics.py`（§9.4/§26.2 `compute_cost_metrics`）、`yield_rates.py`（§9.5/§26.3 `compute_yield_rates`，基于 `yield_funnel_events`）、`budget_evaluation.py`（§9.8 `evaluate_budget`/`period_start`）、`failure_taxonomy.py`（§9.6 `classify_error_code`/`classify_funnel_event`，薄再导出）。
