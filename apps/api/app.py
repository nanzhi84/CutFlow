...
from packages.ai.prompts import (
    ...
    SqlAlchemyPromptRuntimeRepository,
)
...
from packages.core.storage.sqlalchemy_secrets import (
...
from packages.media import SqlAlchemyMediaRepository, UploadReconciler
...
def configure_app_state(app: FastAPI, *, session_factory=None) -> None:
    ...
    app.state.sqlalchemy_case_learning_repository = SqlAlchemyCaseLearningRepository(
        session_factory
    )
    ...
    app.state.upload_reconciler = UploadReconciler(
        app.state.sqlalchemy_upload_repository,
        app.state.object_store,
        app.state.settings.upload,
        owner="api-immediate",
    )
    app.state.sqlalchemy_media_repository = SqlAlchemyMediaRepository(
        session_factory, app.state.object_store
    )
    ...
    app.state.sqlalchemy_secret_repository = SqlAlchemySecretRepository(
        session_factory, app.state.secret_store
    )
    ...
    app.state.sqlalchemy_production_repository = SqlAlchemyProductionRepository(
        session_factory, app.state.object_store
    )
