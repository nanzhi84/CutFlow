...
> 修订 r3（同日）：**取消 V4.5 契约子层**——扩展点是标注 prompt，不是 schema。增强后的 VLM 输出写进现有
> `ClipRetrievalV4.summary`（详细描述）+ `keywords`（实体词），零契约变更零迁移；未来甲方新维度 = 改 prompt 一句话，不加字段。
>
...
- **不加任何契约字段（r3 决定，含不做 V4.5 子层）**：扩展点是标注 prompt 不是 schema——按甲方需求逐个加字段不可持续，架构保持通用统一；VLM 输出全部落现有自由文本字段（`summary`/`keywords`）。
...
5. **S1-5 prompt 序列化与阈值校准**（r2 修订）：
   ...
   - compact 策略重新校准：**簿记字段（帧号/源区间/score）照砍，语义文本全保留**——为 Stage 3 的标注描述进候选字段留好位置。qwen3.7-plus 256k 上下文充足。
...
### Stage 3 — 开放式外观/实体标注增强（核心项，r3：零契约变更）

设计原则（r2/r3 讨论确定）：**充分发挥 VLM 自由文本标注能力**，不设枚举、不做聚类、**不加契约字段**；
扩展点是标注 prompt，不是 schema——未来甲方新维度只改 prompt，不动契约。描述必须一路到达 LLM prompt。

1. **S3-1 VLM 标注 prompt 增强，输出写进现有字段**（`vlm.py:45-141`）：
   - portrait 段重点要求描述人物外观（衣服颜色/款式/材质、发型、配饰、背景环境）；broll 段重点描述实体（产品、品牌露出、场景物件、画面中人物外观）。
   - 落点全部用现有契约：详细描述写进 `ClipRetrievalV4.summary`（自由文本，写细不限长）；实体词并入 `keywords`（自动喂 S1-4 混合检索的关键词通道）；`retrieval_sentence` 保持"一句话检索摘要"职责不稀释。
   - 零契约变更、零迁移、零 openapi 重生成；标注编辑器前端展示 summary 自动带出新内容。每 clip 多产几十至上百字，VLM 成本增幅小。
2. **S3-2 embedding key 与标注解耦**（前置于全库重标注）：已核实索引 embedding 是纯视频输入、文本标注不参与向量（`dashscope.py:265-279`），故 `asset_revision_token` 从 `asset.updated_at` 改绑**媒体内容指纹**（上传 complete 登记的 sha256；`clip_embedding.py:43-48`）。clip 切分变动天然产新 key（key 已含 `source_start/end`+`clip_id`），源文件未换则纯文本重标注**零重嵌入成本**。需 bump `index_version` 一次 + 全库重建（一次性成本换永久解耦）。
3. **S3-3 语义描述进 LLM 候选字段**（增强生效的前提，修 C2）：标注→候选的管道已存在——`broll_pack.py:80-98` 的 `_scene_from_clip` 已把 `retrieval_sentence or summary` 作 `description`、`retrieval.keywords` 作关键词装进候选 metadata；缺的只是 `_compact_prompt_input` 把 description 透传给 LLM（portrait 候选加描述字段，broll 候选在 `scene_name` 旁带 description）。与 S1-5 的"语义全保留"校准配套。
4. **S3-4 重标注策略**：现有字段写富内容，旧标注天然合法无迁移；按需重标走 #168 现成的 per-asset reprocess 通路；批量重标以 annotation `updated_at` / job 记录区分新旧，不需要版本标记字段；解耦（S3-2）落地后批量重标注只有 VLM 成本、无重嵌入成本。

### 落地顺序建议

S1 六项并行可做 → S3-2（解耦，重标注的前置）→ S3-1（标注 prompt 增强）→ S3-3（进 prompt）+ S1-5 配套 → S2-1（query agentic-lite，可与 Stage 3 并行）。

## 4. 依赖与风险

- S2-1 新增 LLM 调用点：一次性调用、产物落 artifact，不破坏确定性；prompt 经 registry 绑定。
- r3 后 Stage 3 零契约变更：无 openapi 重生成、无前端改动、无迁移。代价是 `summary` 字段承担"详细描述"双重职责——`retrieval_sentence` 保持短摘要职责作平衡；若未来 summary 被其他消费方假设为短文本需排查（当前消费方即 broll_pack 的 name/description 回退链，兼容长文本）。
- S3-2 改 embedding key 组成属一次性全库重建，需与 D1（job 状态持久化）先后配合，避免重建中途 API 重启丢进度。
- "衣服尽量相同"这类一致性诉求在本方案下是 **best-effort**（LLM 依据标注描述综合判断），无确定性保证——这是 r2 讨论中有意选择的取舍（不做聚类 key/约束编译）。
...
| 原编号 | 内容 | 不做的理由 |
|---|---|---|
| S3-1(r2) | `ClipEntitiesV45` 契约新子层（visual_detail+entity_keywords） | r3：按甲方需求加字段不可持续，架构保持通用统一；扩展点是标注 prompt，输出落现有 `summary`/`keywords` 零契约变更 |
